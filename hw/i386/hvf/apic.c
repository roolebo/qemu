...
static void hvf_apic_vapic_base_update(APICCommonState *s)
{
    ...
    ret = hv_vmx_vcpu_set_apic_address(CPU(s->cpu), s->vapic_paddr);
    if (ret < 0) {
        frpintf(stderr, "HVF: setting VAPIC address failed (%d)\n",
                ret);
        //fprintf(stderr, "HVF: setting VAPIC address failed (%s)\n",
        //        strerror(-ret));
        abort();
    }
}
...
static uint64_t hvf_apic_mem_read(void *opaque, hwaddr addr,
                                  unsigned size)
{
    return ~(uint64_t)0;
}
...
static void hvf_apic_unrealize(DeviceState *dev, Error **errp)
{
}
